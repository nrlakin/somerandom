...
class TwitterClient(OAuthRemoteApp):
    ...
    def follow(self, user_id):
        ...
        if resp.data.get('errors') is not None:
            self.logger.info("Error following: " + str(data['errors']['message']))
        return resp
    ...
    def get_followers(self):
        ...
        if resp.data.get('errors') is not None:
            self.logger.info("Error getting followers: " + str(data['errors']['message']))
    ...
    def get_followed(self):
        resp = self.get('friends/ids.json',
                data={'screen_name':self.screen_name})
        if resp.data.get('errors') is not None:
            self.logger.info("Error getting followed: " + str(data['errors']['message']))
        return resp.data['ids']

    def post_status(self, status):
        resp = self.post('statuses/update.json', data={'status':status})
        if resp.data.get('errors') is not None:
            self.logger.info("Error posting status: " + str(data['errors']['message']))
        return resp
